...
with Sweep(RUN, Args, Agent) as sweep:
    ...
    Args.tmp_dir = "/home/gridsan/aajay/escher_shared/tmp/"
    Agent.latent_rff = True
    Agent.latent_fourier_features = 2000
    ...
    with sweep.product:
        ...
        Agent.latent_scale = [0.1, 0.5, 1, 3, 5]
        Args.seed = [100, 200, 300, 400, 500]

@sweep.each
def tail(RUN, Args, Agent):
    RUN.prefix, RUN.job_name, _ = RUN(script_path=__file__,
                                      job_name=f"latent-scale-{Agent.latent_scale}/{Args.env_name.split(':')[-1][:-3]}/{Args.seed}")
