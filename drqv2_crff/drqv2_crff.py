...
class WTEncoder(nn.Module):
    def __init__(self, obs_shape, only_low=False):
        super().__init__()
        assert len(obs_shape) == 3
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.wt_filers = wavelets.create_filters(device)
        self.only_low = only_low
        if self.only_low:
            self.repr_dim = 12544//2  # Assumes h = w = 84
        else:
            self.repr_dim = 12544  # Assumes h = w = 84

        self.convnet_lo = nn.Sequential(nn.Conv2d(obs_shape[0], 32, 3, stride=2),
        ...
        if not self.only_low:
            self.convnet_hi = nn.Sequential(nn.Conv2d(3*obs_shape[0], 32, 3, stride=2),
                                        nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                        nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                        nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                        nn.ReLU())
    ...
    def forward(self, obs):
        # Obs is (N, C, H, W)
        obs = obs / 255.0 - 0.5
        obs = wavelets.wt(obs, self.wt_filers)
        _, _, h, w = obs.size()
        ...
        if not self.only_low:
            obs_rest = torch.cat([obs2, obs3, obs4], dim=1)

        h_lo = self.convnet_lo(obs1)
        h_lo = h_lo.view(h_lo.shape[0], -1)

        if not self.only_low:
            h_hi = self.convnet_hi(obs_rest)
            h_hi = h_hi.view(h_hi.shape[0], -1)
            h = torch.cat([h_lo, h_hi], dim=1)
        else:
            h = h_lo
...
class Actor(nn.Module):
    def __init__(self, repr_dim, action_shape, feature_dim, hidden_dim, latent_rff=False, latent_fourier_features=None, scale=None):
        super().__init__()

        if latent_rff:
            self.trunk = nn.Sequential(nn.Linear(repr_dim, feature_dim),
                                       nn.LayerNorm(feature_dim), nn.Tanh(),
                                       models.LFF(feature_dim, latent_fourier_features, scale=scale),)
            self.policy = nn.Sequential(nn.Linear(latent_fourier_features, hidden_dim),
    ...
    def __init__(self, repr_dim, action_shape, feature_dim, hidden_dim, latent_rff=False, latent_fourier_features=None, scale=None):
        ...
        if latent_rff:
            ...
            self.Q1 = nn.Sequential(
                nn.Linear(latent_fourier_features + action_shape[0], hidden_dim),
...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 stddev_schedule, stddev_clip, wavelet_transform = False, wavelet_only_low = False,
                 rff=False, conv_fourier_features=None, scale=None,
                 latent_rff=False, latent_fourier_features=None, latent_scale=None):
        self.device = device
        ...
        if wavelet_transform:
            self.encoder = WTEncoder(obs_shape, only_low=wavelet_only_low).to(device)
        else:
            self.encoder = Encoder(obs_shape, fourier_features=conv_fourier_features, scale=scale, rff=rff).to(device)

        self.actor = Actor(self.encoder.repr_dim, action_shape, feature_dim,
                           hidden_dim, latent_rff=latent_rff, scale=latent_scale, latent_fourier_features=latent_fourier_features).to(device)
